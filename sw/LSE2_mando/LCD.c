...
// Funcion para escribir una cadena (if print true white, if false black)
...
void hsv2rgb(uint16_t h, uint8_t* Re, uint8_t* Gr, uint8_t* Bl){ //h goes from 0 to 360
    ...
    if (h < 60) {
        ...
        r = 0;
        g = 0;
        b = 0;  // Este caso no deberia ocurrir con el valor de H en el rango [0, 360)
    }
...
void floatToString(float number, char *result) {
    // Determinar si el numero es negativo y tomar el valor absoluto
    ...
    // Ajustar el numero para que solo tenga un digito en la parte entera
    if (integerPart >= 10) {
    ...
    // Convertir cada uno de los 5 d�gitos decimales a caracteres
    int i;
    for (i = 4; i >= 0; i--) {
